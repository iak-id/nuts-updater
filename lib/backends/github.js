...
GitHubBackend.prototype.getAssetStream = function(asset) {
    ...
    var gotOptions = { headers: headers };

    if (this.opts.token) {
        headers['Authorization'] = 'token '+this.opts.token;
    } else if (this.opts.username) {
        gotOptions.username = this.opts.username;
        gotOptions.password = this.opts.password;
    }

    return Q(got.stream(asset.raw.url, gotOptions));
};
